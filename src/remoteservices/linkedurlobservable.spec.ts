...
import getLinkedUrlObservables, * as linkedUrlObservable from "./linkedurlobservable";
...
describe("linkedurlobservable", () => {
    ...
    it("can load chained links", async () => {
        const fetchFunction: linkedUrlObservable.FetchFunction<{ block: string }> =
            (url) => Promise.resolve(switchcase({
    ...
    it("can load limited chained links", async () => {
        const fetchFunction: linkedUrlObservable.FetchFunction<{ block: string }> =
            (url) => Promise.resolve(switchcase({
                "http://localhost:9999": { content: data0, linkHeader: "</page1>; rel=next, </page2>; rel=last" },
                "http://localhost:9999/page1": { content: data1, linkHeader: "</page2>; rel=next, </page2>; rel=last" },
        ...
        linkedUrlObservable.setMaxPages(2);
        const result = await getLinkedUrlObservables("http://localhost:9999", fetchFunction)
            .toArray()
            .toPromise();
        assert.deepEqual(result, [data0, data1]);
    });
});
